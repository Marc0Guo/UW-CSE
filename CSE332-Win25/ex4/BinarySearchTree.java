...
public class BinarySearchTree<K extends Comparable<K>, V> implements OrderedDeletelessDictionary<K,V> {
    ...
    public K findNextKey(K key){
        ...
        while (curr != null) {
            if (key.compareTo(curr.key) < 0) { //give key smaller than current key
                next = curr;
                curr = curr.left; // go left
            } else { // given key greater than current key
    ...
    public K findPrevKey(K key){
        ...
        TreeNode<K, V> next = null;
        while (curr != null) {
            if (key.compareTo(curr.key) > 0) { //give key greater than current key
                next = curr;
                curr = curr.right; // go right
            } else { // given key smaller than current key
                curr = curr.left; // go left
            }
        }
        return (next != null) ? next.key : null;
