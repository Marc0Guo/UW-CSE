...
public class RangeTree{
    ...
    public boolean hasConflict(Range query) {
        ...
        // Find the largest start key smaller than query end
        Double Start = byStart.findPrevKey(query.end);
        if (Start != null) {
            Range range = byStart.find(Start);
            if (range.end > query.start) {
                return true; // overlap if range ends after query start
            }
        }

        //Find the smallest end key greater than quarty start
        Double End = byEnd.findNextKey(query.start);
        if (End != null) {
            Range range = byEnd.find(End);
            if (range.start < query.end) {
                return true; // overlap if range start before query end
            }
        }
